...
const FilterBarOption: FC<FilterBarProps> = ({name, label, onRemove}) => {
    return ( 
        <li data-option-name={name} className={styles.filterbarBarOption}>
            <span className={styles.sboText}>{label}</span>
            <button className={styles.sboCloseBtn} onClick={(e: PointerEvent<HTMLButtonElement>) => {e.stopPropagation(); onRemove(name)}}>
